...
- Random holdout v0.4：事前冻结后新增 12 篇/169 页/157 个相邻页对；0 个路由候选下
  ...
  规则在 gold 签署前保持冻结；该批现在可明确转为开发/校准集，详见
...
把已完成人工 gold 的 v0.4 明确转为开发/校准集，加入多页 Figure 面板连续性证据，保持
旧困难负例回归；随后从候选位置 38 继续建立新的独立 holdout。v0.4 只保留冻结基线的
独立结论，不能再用来宣称修正规则的泛化通过。
