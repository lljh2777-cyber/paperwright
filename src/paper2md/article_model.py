...
ARTICLE_MODEL_CONTRACT_VERSION = "paper2md-article-model-v0.1"
...
def _render_article_markdown(value: Mapping[str, Any]) -> str:
    ...
    for block in value["blocks"]:
        ...
        if block["kind"] == "visual_slot":
            marker = (
                f'<!-- p2md:slot id="{block_id}" '
                f'asset="{block["asset_id"]}" -->'
            )
        else:
            marker = (
                f'<!-- p2md:block id="{block_id}" '
                f'kind="{block["kind"]}" -->'
            )
...
def _reader_index(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for block in value["blocks"]:
        block_id = block["id"]
        syntax = "p2md:slot" if block["kind"] == "visual_slot" else "p2md:block"
        blocks.append(
            {
                "id": block_id,
                "kind": block["kind"],
