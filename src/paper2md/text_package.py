...
TEXT_PACKAGE_VALIDATION_VERSION = "paper2md-text-package-validation-v0.1"
_ARTICLE_PATH = "article.md"
_MODEL_PATH = "_paper2md/article-model.json"
_READER_PATH = "_paper2md/reader.json"
_MANIFEST_PATH = "_paper2md/manifest.json"
_TASK_PATH = "_paper2md/06-text-review/text-task.json"
_REVIEW_PATH = "_paper2md/06-text-review/text-review.json"
_VALIDATION_PATH = "_paper2md/06-text-review/validation-report.json"
_VALIDATION_MARKDOWN_PATH = "_paper2md/06-text-review/validation-report.md"
...
def _load_source_package(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if not manifest_path.is_file() or manifest_path.is_symlink():
        raise ConfigurationError("源文档包缺少 _paper2md/manifest.json")
...
def _validation_markdown(value: Mapping[str, Any]) -> str:
    checks = value["checks"]
    lines = [
        "# Paper2MD Text Review Validation",
        "",
...
def build_text_reviewed_package(
    ...
    temporary = Path(
        tempfile.mkdtemp(
            prefix=f".{target.name}.paper2md-text-",
            dir=target.parent,
        )
    )
    try:
