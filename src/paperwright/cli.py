"""PaperWright Alpha command line interface."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    validate_reader.add_argument(
        "--package-root",
        type=Path,
        help="默认从 _paperwright/reader.json 推断文档包根目录",
    )
    ...
    apply_layout.add_argument(
        "--include-source-pdf",
        action="store_true",
        help="将原 PDF 复制到 _paperwright/source.pdf；默认只记录路径和哈希",
    )
    apply_layout.add_argument(
        "--extraction-profile",
        choices=("fast", "standard", "forensic"),
...
def _validate_reader(path: Path, package_root: Path | None) -> int:
    ...
    root = (
        ...
        if source.parent.name == "_paperwright"
        else source.parent
    )
    try:
...
def _product(config) -> PaperWright:
    product = PaperWright(config=config)
