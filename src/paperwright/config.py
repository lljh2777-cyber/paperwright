...
@dataclass(frozen=True)
class PaperWrightConfig:
    backend: str = "pdfium"
    contract_version: str = "paperwright-physical-document-v0.2"
    ...
    def validate(self) -> None:
        ...
        if self.contract_version != "paperwright-physical-document-v0.2":
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> "PaperWrightConfig":
        allowed = {
            "backend",
            "contract_version",
            "limits",
...
def load_config(path: str | Path | None) -> PaperWrightConfig:
    """Load strict JSON configuration; defaults apply when path is omitted."""

    if path is None:
        return PaperWrightConfig()
    ...
    return PaperWrightConfig.from_dict(value)


def with_cli_overrides(
    base: PaperWrightConfig,
    *,
