"""Strict validation for the PaperWright reader interoperability contract."""
...
READER_CONTRACT_VERSION = "paperwright-reader-v0.1"
MARKDOWN_ANCHOR_CONTRACT_VERSION = "paperwright-markdown-anchor-v0.1"
BLOCK_FINGERPRINT_VERSION = "paperwright-visible-block-fingerprint-v0.1"
...
_PUBLIC_ANCHOR_RE = re.compile(
    r'^<!-- pwwd:(?P<syntax>block|slot) id="(?P<id>[a-z]+_[0-9a-f]{24})"'
    r'(?: kind="(?P<kind>[a-z_]+)")?'
...
def stable_reader_id(prefix: str, source_sha256: str, payload: object) -> str:
    digest = hashlib.sha256(
        (
            f"paperwright-reader-id-v0.1\0{source_sha256}\0"
