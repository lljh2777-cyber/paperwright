#!/usr/bin/env python3
"""Build and independently verify the deterministic Phase 4 auto handoff."""
...
DELIVERY = WORKSPACE / "phase4-auto-region-delivery"
SOURCE_COPY = DELIVERY / "Paper2MD"
ZIP_PATH = WORKSPACE / "phase4-auto-region-src.zip"
PATCH_PATH = WORKSPACE / "phase4-auto-region-changes.patch"
BASE = "25e4ecea02979cf7dcb56ab2d280425bc56e74e2"
...
def role(relative: str) -> str:
    ...
    if relative.startswith("phase4_auto_region/"):
        return "phase4_auto_small_machine_or_human_evidence"
    if relative.startswith(
        ("stage_a/", "stage_b/", "realworld/", "phase3/", "phase4_render_spike/")
    ):
...
def fresh_tree_verify(patch_path: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="paper2md-phase4-auto-fresh-"
    ) as temporary:
...
def main() -> int:
    if git("rev-parse", "HEAD").strip() != BASE:
        raise RuntimeError("HEAD is not the authorized Phase 4 spike commit")
    preserved = git(
        "diff",
        "--name-only",
        BASE,
    ...
    required = (
        "phase4_auto_region/baseline_authority.json",
        "phase4_auto_region/frozen_rules_v1.json",
        "phase4_auto_region/frozen_rules_v1_1_amendment.json",
        "phase4_auto_region/fixture_matrix_v1.json",
        "phase4_auto_region/auto_region_summary.json",
        "phase4_auto_region/auto_candidate_inventory.json",
        "phase4_auto_region/visual_review_inventory.json",
        "phase4_auto_region/test_summary.json",
        "phase4_auto_region/report_zh.md",
        "phase4_auto_region/REPRODUCE.md",
    )
    ...
    with tempfile.TemporaryDirectory(
        prefix="paper2md-phase4-auto-index-"
    ) as temporary:
    ...
    for name, source in {
        "test_summary.json": "phase4_auto_region/test_summary.json",
        "auto_region_summary.json": "phase4_auto_region/auto_region_summary.json",
        "visual_review_inventory.json": (
            "phase4_auto_region/visual_review_inventory.json"
        ),
        "report_zh.md": "phase4_auto_region/report_zh.md",
        "REPRODUCE.md": "phase4_auto_region/REPRODUCE.md",
    }.items():
    ...
    (DELIVERY / "PACKAGE_CONTENTS.txt").write_text(
        "\n".join(
            (
                ...
                "fresh_tree_verification.json - 全新 worktree patch 与测试结果",
                "test_summary.json / auto_region_summary.json - 机器摘要",
                "visual_review_inventory.json - 无图视觉审查索引",
                "report_zh.md / REPRODUCE.md - 中文报告和复现命令",
                "真实 PDF、图片、转换输出、binary、JAR、缓存与凭据均不在包内。",
            )
        )
        + "\n",
        encoding="utf-8",
    ...
    policy_process = run(
        [
            sys.executable,
            str(ROOT / "tools/check_repo_policy.py"),
            "--root",
            str(SOURCE_COPY),
        ],
        cwd=SOURCE_COPY,
    ...
    manifest = {
        "schema_version": "paper2md-phase4-auto-stage-manifest-v1",
        "base_commit": BASE,
        ...
        "package_exclusions": [
            "PDF/corpus/gold",
            "real paper images and conversion output",
            "PDFium/JAR/binary",
            "cache/venv/node_modules",
            "credentials/PII",
        ],
    }
    ...
    with tempfile.TemporaryDirectory(
        prefix="paper2md-phase4-auto-zip-"
    ) as temporary:
