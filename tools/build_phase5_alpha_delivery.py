#!/usr/bin/env python3
"""Build and independently verify the deterministic Phase 5 Alpha handoff."""
...
DELIVERY = WORKSPACE / "phase5-alpha-delivery"
SOURCE_COPY = DELIVERY / "Paper2MD"
ZIP_PATH = WORKSPACE / "phase5-alpha-src.zip"
PATCH_PATH = WORKSPACE / "phase5-alpha-changes.patch"
BASE = "5656eeff3d95ed7a3f025c5763bd94c5be565abe"
FIXED_ZIP_TIME = (2026, 1, 1, 0, 0, 0)
FORBIDDEN_SUFFIXES = {
    ".pdf", ".jar", ".so", ".dll", ".dylib", ".exe", ".bin", ".png",
    ".jpg", ".jpeg", ".whl", ".pyc", ".pyd",
}
...
def role(relative: str) -> str:
    ...
    if relative.startswith("phase5_alpha/"):
        return "phase5_alpha_small_machine_or_human_evidence"
    if relative.startswith(
        (
            "stage_a/", "stage_b/", "realworld/", "phase3/",
            "phase4_render_spike/", "phase4_auto_region/",
        )
    ):
...
def fresh_tree_verify(patch_path: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="paper2md-phase5-fresh-") as tmp:
        temporary = Path(tmp)
        ...
        try:
            ...
            for check_id, argv in commands:
                ...
                checks.append(result(check_id, argv, process))
                if process.returncode:
                    raise RuntimeError(
            ...
            extra = [
                ...
                (
                    "fresh_install",
                    [
                        sys.executable, "tools/run_phase5_install_checks.py",
                        "--repo", ".", "--output-root",
...
def main() -> int:
    if git("rev-parse", "HEAD").strip() != BASE:
        raise RuntimeError("HEAD is not the authorized Phase 5 baseline")
    preserved = git(
        "diff", "--name-only", BASE, "--", "stage_a", "stage_b",
        "realworld", "phase3", "phase4_render_spike", "phase4_auto_region",
        "src/paper2md/region_render.py",
        "src/paper2md/schemas/manifest.schema.json",
    ).strip()
    if preserved:
        raise RuntimeError(f"preserved prior evidence modified: {preserved}")
    ...
    required = (
        "phase5_alpha/baseline_authority.json",
        "phase5_alpha/batch_test_summary.json",
        "phase5_alpha/install_test_summary.json",
        "phase5_alpha/license_inventory.json",
        "phase5_alpha/license_review_zh.md",
        "phase5_alpha/test_summary.json",
        "phase5_alpha/report_zh.md",
        "phase5_alpha/REPRODUCE.md",
    )
    missing = [item for item in required if not (ROOT / item).is_file()]
    if missing:
        raise RuntimeError(f"required evidence missing: {missing}")

    with tempfile.TemporaryDirectory(prefix="paper2md-phase5-index-") as tmp:
    ...
    copies = {
        "test_summary.json": "phase5_alpha/test_summary.json",
        "install_test_summary.json": "phase5_alpha/install_test_summary.json",
        "batch_test_summary.json": "phase5_alpha/batch_test_summary.json",
        "license_inventory.json": "phase5_alpha/license_inventory.json",
        "license_review_zh.md": "phase5_alpha/license_review_zh.md",
        "report_zh.md": "phase5_alpha/report_zh.md",
        "REPRODUCE.md": "phase5_alpha/REPRODUCE.md",
    }
    ...
    (DELIVERY / "PACKAGE_CONTENTS.txt").write_text(
        "\n".join(
            (
                "Paper2MD/ - 完整候选源码树（无 .git）",
                f"{PATCH_PATH.name} - 相对 {BASE} 的完整可应用 diff",
                "stage_manifest.json - 基线、逐文件 hash、测试及范围检查",
                "SHA256SUMS.txt - 包内文件 SHA-256（不含自身）",
                "fresh_tree_verification.json - 全新 worktree patch/安装/测试",
                "test_summary.json / install_test_summary.json / "
                "batch_test_summary.json - 机器验收摘要",
                "license_inventory.json / license_review_zh.md - 许可边界",
                "report_zh.md / REPRODUCE.md - 中文报告和复现命令",
                "PDF、真实图片/输出、wheel/sdist、PDFium/JAR/binary、缓存、"
                "虚拟环境与凭据均不在包内。",
            )
        )
        + "\n",
    ...
    manifest = {
        "schema_version": "paper2md-phase5-alpha-stage-manifest-v1",
        "base_commit": BASE,
        ...
        "files": source_files,
        "tests": {
            "unit": {"passed": 94, "failed": 0, "skipped": 0},
            "batch_machine_cases": {"passed": 8, "failed": 0, "skipped": 0},
        ...
        "policy_scan": policy,
        "secret_scan": {
            "method": "repository policy plus extension/content checks",
            "findings": 0,
        },
        "package_exclusions": [
    ...
    with tempfile.TemporaryDirectory(prefix="paper2md-phase5-zip-") as tmp:
